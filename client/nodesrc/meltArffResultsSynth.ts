...
function meltArffData(baseDir: string, resultFileName: string = "allResults", filterFunc: (file: string) => boolean = (file: string) => true) {
  let allResults: any[][] = [];

  console.log(`[${moment().format()}] processing ${baseDir}`);

  const fileName = `${baseDir}${resultFileName}.csv`;
  ...
  fs.readdirSync(baseDir)
    ...
    .forEach(file => {
      ...
      const learnerConfig = file.replace(/[a-z.]/ig, "");
...
// meltArffData("./nodesrc/synthetic/stationaryDist5F7L/adaptivePLT/", "all20000Results", (file: string) => file.indexOf("20000") > 1);
meltArffData("./nodesrc/synthetic/stationaryDist5F7L/plt/", "all20000Results-PLT", (file: string) => file.indexOf("20000") > 1);
